<template>
	<div class="c-single-select">
		<i-form-item prop="title" class="_hide-required">
			<span slot="label" class="g-c-red-mid">【多选题】</span>
			<oa-limit-words
		...
		<i-checkbox-group v-model="formData.value" class="g-m-l-20">
			<vc-sort-list
				...
				<template slot-scope="option">
					...
					>
						<i-input 
							...
							placeholder="请输入选项"
						/>
						<i-checkbox :label="option.value" class="g-m-l-15" >
							<span style="margin-left:10px !important" class="g-c-black2" >设为答案</span>
						</i-checkbox>
						<i 
		...
		<i-button 
			...
			style="width:300px;margin-left:110px" 
...
export default {
	...
	methods: {
		...
		handleRemoveOption(option) {
			...
			const index2 = this.formData.value.findIndex(item => item == option.value);
			if (index2 > -1) this.formData.value.splice(index2, 1);
