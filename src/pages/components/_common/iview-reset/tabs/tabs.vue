<template>
	<div :class="classes">
		<div :class="[prefixCls + '-bar']">
			<div v-if="showSlot" :class="[prefixCls + '-nav-right']"><slot name="extra"/></div>
			<div
				ref="navContainer"
			...
			>
				<div ref="navWrap" :class="[prefixCls + '-nav-wrap', scrollable ? prefixCls + '-nav-scrollable' : '']">
					...
					<div ref="navScroll" :class="[prefixCls + '-nav-scroll']">
						<div ref="nav" :class="[prefixCls + '-nav']" :style="navStyle" class="nav-text">
							<div :class="barClasses" :style="barStyle"/>
							<div v-for="(item, index) in navList" :key="index" :class="tabCls(item)" @click="handleChange(index)">
								<Icon v-if="item.icon !== ''" :type="item.icon"/>
								<Render v-if="item.labelType === 'function'" :render="item.label"/>
								<template v-else>{{ item.label }}</template>
								<Icon v-if="showClose(item)" type="ios-close" @click.native.stop="handleRemove(index)"/>
							</div>
						</div>
					</div>
				</div>
		...
		<div ref="panes" :class="contentClasses" :style="contentStyle"><slot/></div>
	</div>
</template>
<script>
...
import { oneOf, MutationObserver, findComponentsDownward } from 'iview/src/utils/assist';
...
export default {
	...
	methods: {
		getTabs() { 
			...
			const AllTabPanes = findComponentsDownward(this, 'tab-pane');
			const TabPanes = [];
		...
		updateVisibility(index) {
			[...this.$refs.panes.querySelectorAll(`.${prefixCls}-tabpane`)].forEach((el, i) => {
				if (index === i) {
					[...el.children].filter(child => child.classList.contains(`${prefixCls}-tabpane`)).forEach(
