		...
		<div :class="`${prefix}-wrapper`" :style="wrapperStyle">
			<div :class="`${prefix}`">
				<div :class="`${prefix}-ink`">
					<span v-show="showInk" :class="`${prefix}-ink-ball`" :style="{top: `${inkTop}px`}"/>
				</div>
				<slot/>
			</div>
		</div>
...
<script>
import { scrollTop, findComponentsDownward, sharpMatcherRegx } from 'iview/src/utils/assist';
...
export default {
	...
	methods: {
		...
		handleScroll(e) {
			...
			const scrollTop_ = document.documentElement.scrollTop || document.body.scrollTop || e.target.scrollTop;
			this.getCurrentScrollAtTitleId(scrollTop_);
		},
		handleHashChange() {
			const url = window.location.href;
			const linkName = this.$route.query[this.queryName] || '';
			if (!linkName) return;
			this.currentLink = linkName;
			this.currentId = linkName;
		},
		handleScrollTo() {
		...
		updateTitleOffset() {
			const links = findComponentsDownward(this, 'anchor-link').map(link => {
				return link.href;
			});
		...
		getCurrentScrollAtTitleId(scrollTop_) {
			let i = -1;
			...
			scrollTop_ += this.bounds;
			while (++i < len) {
				let currentEle = this.titlesOffsetArr[i];
				let nextEle = this.titlesOffsetArr[i + 1];
				if (scrollTop_ >= currentEle.offset && scrollTop_ < ((nextEle && nextEle.offset) || Infinity)) {
