...
import { getConstructUrl, getParseUrl, cloneDeepEasier } from 'wya-vc/lib/utils/utils';
...
export default {
	...
	data() {
		...
		return {
			...
			// 内部不直接修改外部的值
			rebuildData: {},
			tableOptsVC: this.$vc.config.Paging.tableOpts || {}
		}; 
	},
	computed: {
		data() {
	...
	watch: {
		show(v) {
			if (v) {
				...
				if (v && this.$refs.tableTarget) {
					this.$refs.tableTarget.handleResize();
				}
			}
		},
		/**
	...
	created() {
		...
		let page = this.history ? getParseUrl().query.page || 1 : 1;
	...
	methods: {
		...
		expand(opts = {}) {
			...
			if (children.length === 0 && this.loadExpandData) {
				...
				if (!this.loadingExpand && load && load.then) {
					...
					load.then((children_) => {
						if (children_ instanceof Array) {
							children_ = this.makeRebuildDataForExpand(__level__ + 1, children_);

							// 同步到data中	
							targetArr[index].__expand__ = true;
							targetArr[index].children = children_;
							targetArr.splice(index + 1, 0, ...this.getLinearArray(children_));
		...
		emitExpand(opts = {}) {
			...
			this.$emit('expand', { 
				...
				maxLevel,
				callback: (opts_ = {}) => {
					const { selected, all = false } = opts_;
