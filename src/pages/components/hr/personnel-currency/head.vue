<template>
	<div class="v-hr-pesonnel-currency-head g-flex-ac">
		...
		<div class="_detail g-flex-ac ">
			<div :style="`background:url(${OSS_PROP_ICON1})`" class="_coin g-m-r-10 g-m-l-20"/>
			<div class="g-m-r-20">
				<div class="g-fs-24 g-c-green-light" >{{ baseInfo.settled_coins }}</div>
				<div class="g-fs-14 g-c-black4">可用余额</div>
			</div>
			<i-button @click="()=>{$router.push('/sale/property/center')}">去消费</i-button>
			<span style="margin-right:80px" class="_border g-m-l-20"/>
			<div :style="`background:url(${OSS_PROP_ICON1})`" class="_coin g-m-r-10"/>
			<div class="g-m-r-20">
				<div class="g-fs-24" style="color:#E74854;" >{{ baseInfo.settling_coins }}</div>
				<div class="g-fs-14 g-c-black4">待结算</div>
			</div>
		</div>
		<div class="_policy g-flex-cc">
			<span class="__circle" style="top:-9px"/>
			<span class="__circle" style="bottom:-9px"/>
			<i-button type="primary" @click="handleGoDetail">微币奖惩政策</i-button>
		</div>
	</div>
</template>
...
export default {
	...
	methods: {
		...
		handleGoDetail() {
			let { query } = this.$route;
			this.$router.push({ path: '/hr/personnel/currency/rewards', query: { ...query } });
		}
	}
};
</script>
