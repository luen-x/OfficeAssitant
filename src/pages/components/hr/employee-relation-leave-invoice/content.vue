<template>
	<div class="v-hr-employee-relation-leave-invoice g-m-t-20 ">
			...
			<i-tab-pane
				...
				<div v-if="is_apply && item.value === '1'" class="_apply">
					<vc-print ref="applyTarget">
						<oa-print-dimission-apply :info="applyInfo" />
					</vc-print>
				</div>
				<div v-if="is_interview && item.value === '2'" class="_talk">
					<vc-print ref="talkTarget">
						<oa-print-dimission-talk :info="talkInfo" />
					</vc-print>
				</div>
				<div
...
import PrintDimissionApply from '../form/print/dimission-apply';
import PrintDimissionTalk from '../form/print/dimission-talk';
...
export default {
	...
	methods: {
		...
		loadFile() {
			this.$request({
				...
				if (this.fileInfo.length) {
					this.file = this.fileInfo.map(item => item.url);
		...
		loadTab() {
			...
			// 	type: "POST",
			// 	param: {
			// 		relation_id: this.$route.query.leave_id,
			// 		scenario: 5
			// 	},
			// 	loading: false
			// }).then((res) => {
			...
			// 			item.type === 2 ? this.apply_id = item.staff_template_id : this.talk_id = item.staff_template_id;
			// 		});
			setTimeout(() => {
				this.loadApplyInfo();
				this.loadTalkInfo();
...
.v-hr-employee-relation-leave-invoice {
	._apply, ._talk {
		// width: 90%;
		margin-left: auto;
		margin-right: auto;
