<template>
	<div v-if="isFetched" class="v-hr-salary-base">
...
export default {
	...
	data() {
		...
		return {
			isAll: false,
			depart_id: String(query.depart_id || "1"), // 同tabs下的value
			current: {},
	...
	computed: {
		isFetched() {
			return this.$store.state.hrSalaryBase.isFetched;
		},
		hasSetting() {
			return this.$store.state.hrSalaryBase.hasSetting;
		},
		tabs() {
			return this.$store.state.hrSalaryBase.tabs || [];
		},
		listInfo() {
			return this.$store.state.hrSalaryBase.listInfo;
		}
	},
	watch: {
		tabs(newVal, oldVal) {
	...
	methods: {
		...
		handleChange(departId) {
			if (this.depart_id !== departId) {
				...
				this.$router.replace(getHashUrl('/hr/salary/base', { ...query }));
			}
		},
		...
		handleMultiDelete() {
			if (this.selected.length) {
				// 获取被选底薪制度id数组
				let arr = this.selected.map(sed => sed.basic_salary_id);
				...
				PModal.popup({
					...
					this.isAll = false;
					res === 1 ? this.$store.commit('HR_SALARY_BASE_LIST_INIT', this.depart_id) : '';
				}).catch(() => {});
			} else {
