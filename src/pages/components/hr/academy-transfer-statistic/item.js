...
import { modifyPModal } from './popup/modify.vue';
...
export default {
	...
	methods: {
		...
		// 查看
		handleClick(sed) {
			this.$router.push(`/hr/academy/transfer/teacher/detail?project_id=${sed.project_id}`);
		},
		handleClickItem(name, params) {
			if (name === 'modify') {
				modifyPModal.popup({
					info: params
				}).then(res => {
				...
				this.$router.push(getHashUrl(
					...
					{
						month: this.$route.query.try_month ? this.$route.query.try_month : moment().format("YYYY-MM"),
			...
			let demandColumns = [
				...
				},
				{
					...
					render: (h, params) => {
						...
						return (
							<div>
								{ (!params.row.children && params.row.need_number !== 0) && <Dropdown
									transfer
									placement="left-start"
									onOn-click={(name) => this.handleClickItem(name, params.row)}>
									<i class="iconfont icon-point g-fs-20 g-pointer" />
									<DropdownMenu slot="list">
										{ this.$auth[952] ? <DropdownItem name="modify">修改人员需求</DropdownItem> : null }
										{ this.$auth[953] ? <DropdownItem name="watch">查看详细信息</DropdownItem> : null }
									</DropdownMenu>
								</Dropdown>}
							</div>
						);
