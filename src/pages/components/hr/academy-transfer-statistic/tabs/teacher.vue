<template>
	<div class="v-hr-academy-transfer-statistic-teacher">
		...
		<vc-paging
			...
			class="v-hr-academy-transfer-statistic-teacher-list g-m-t-20"
			mode="table"
...
export default {
	...
	computed: {
		listInfo() {
			return this.$store.state.hrAcademyTransferStatistic.teachList;
		}
	},
	mounted() {
	...
	methods: {
		loadData(page, pageSize) {
			let { query = {} } = getParseUrl();
			return this.request({
				url: 'HR_ACADEMY_TRANSFER_STATISTIC_TEACHER_LIST_GET',
				type: 'GET',
				param: {
