<template>
	<div>
		...
		<div class="v-hr-train-provide-setting">
			<div class="__left">
				<div class="__text">
					<div v-if="relation_id.length">
						<div v-for="(course, i) in relation_id" :key="i">{{ course.course_name }}</div>
					</div>
					<div v-else>关联项目</div>
				</div>
...
export default {
	...
	methods: {
		handleDownload() {
			...
			window.open(getHashUrl(API_ROOT._HR_ACADEMY_TRAIN_QR_CODE_DOWNLOAD_GET, {
