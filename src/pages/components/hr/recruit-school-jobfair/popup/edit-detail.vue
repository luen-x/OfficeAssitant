	...
	<div class="v-hr-recruit-school-jobfair-edit">
		<div class="_wrapper g-flex-cc g-jc-c">
			...
			>
				...
				<i-form-item 
					:rules="formValidate.status == 2 ? ruleValidate.action_time : { type: 'array' }"
					prop="action_time"
					label="招聘会时间：" >
					<i-date-picker
	...
	methods: {
		...
		getInfoData() {
			this.$request({
				...
				this.formValidate.school_id = res.data.school_id;
				...
				this.formValidate.meeting_year = +res.data.list[0].meeting_year;
				this.formValidate.season_type = res.data.list[0].season_type;
		...
		handleSave() {
			return this.$refs.form.validate((valid) => {
				...
				let time2 = this.formValidate.action_time[0] && moment(this.formValidate.action_time[1]).format('YYYY-MM-DD HH:mm:ss');
				timeArr.push(time1);
				timeArr.push(time2);
