		...
		>
			...
			>
				...
				>
					...
					<div v-if="$auth[1143]" slot="extra" class="g-flex-ac">
						<i-checkbox
							v-model="isAll"
...
export default {
	...
	data() {
		...
		return {
			isAll: false,
			depart_id: String(query.depart_id || "11"), // 同tabs下的value
			current: {},
	...
	computed: {
		isFetched() {
			return this.$store.state.hrSalaryTeam.isFetched;
		},
		hasSetting() {
			return this.$store.state.hrSalaryTeam.hasSetting;
		},
		tabs() {
			return this.$store.state.hrSalaryTeam.tabs;
		},
		listInfo() {
			return this.$store.state.hrSalaryTeam.listInfo;
		}
	},
	watch: {
		tabs(newVal, oldVal) {
	...
	methods: {
		...
		handleChange(departId) {
			if (this.depart_id !== departId) {
				...
				this.$router.replace(getHashUrl('/hr/salary/team', { ...query }));
			}
		},
		handleChangePageSize() {
			this.isAll = false;
			this.$store.commit('HR_SALARY_TEAM_LIST_INIT');
		},
			...
			if (this.selected.length) {
				...
				DelTipModal.popup({
					...
					this.$request({
						...
						this.$store.commit('HR_SALARY_TEAM_LIST_INIT', this.depart_id);
					}).catch(err => {});
				}).catch(() => {});
			} else {
