<template>
	<div class="v-hr-employee-rank-title">

		<div class="g-flex g-jc-sb js-filter">
		...
		>
			<i-tab-pane
				...
				<vc-paging
					...
					class="v-hr-employee-rank-title-list g-m-t-20"
					mode="table"
...
export default {
	...
	computed: {
		stairList() {
			return this.$store.state.hrEmployeeRankTitle.stairList;
		},
		stair_depart_id() {
			return this.$store.state.hrEmployeeRankTitle.stair_depart_id;
		},
		tabs() {
			return this.$store.state.hrEmployeeRankTitle.tabs;
		},
		listInfo() {
			return this.$store.state.hrEmployeeRankTitle.listInfo;
		}
	},
	watch: {
		// 一级部门
		stairList(newVal, oldVal) {
			const { query } = this.$route;
			this.$store.commit('HR_EMPLOYEE_RANK_TITLE_SET_STAIR', query.stair_depart_id ? +query.stair_depart_id : newVal[0].depart_id);
	...
	methods: {
		loadData(page, pageSize) {
			...
			return this.request({
				url: 'HR_EMPLOYEE_RANK_TITLE_LIST_GET',
				type: 'GET',
				param: {
					...query,
		...
		handleStairChange(sed) {
			this.$store.commit('HR_EMPLOYEE_RANK_TITLE_SET_STAIR', sed);
			// 加载tab
			...
			this.$router.replace(
				getHashUrl("/hr/employee/rank/title", { ...query })
			);
		...
		handleChange(rankSystemId) {
			...
			this.$router.replace(getHashUrl('/hr/employee/rank/title', { ...query }));
		},
		// 阶段发生变化获取相应级称列表
		...
		handleOutput() {
			...
			const url = getHashUrl(API_ROOT['HR_EMPLOYEE_RANK_TITLE_LIST_GET'], {
				...query,
				rank_system_id: this.rank_system_id,
				export: 1,
				type: 2,
