<template>
	<div v-if="isFetched" class="v-hr-salary-per">
...
export default {
	...
	computed: {
		isFetched() {
			return this.$store.state.hrSalaryPer.isFetched;
		},
		hasSetting() {
			return this.$store.state.hrSalaryPer.hasSetting;
		},
		tabs() {
			return this.$store.state.hrSalaryPer.tabs;
		},
		listInfo() {
			return this.$store.state.hrSalaryPer.listInfo;
		}
	},
	watch: {
		tabs(newVal, oldVal) {
	...
	methods: {
		...
		handleChange(departId) {
			if (this.depart_id !== departId) {
				...
				this.$router.replace(getHashUrl('/hr/salary/per', { ...query }));
			}
		},
		handleChangePageSize() {
			this.isAll = false;
			this.$store.commit('HR_SALARY_PER_LIST_INIT', this.depart_id);
		},
		...
		handleMultiDelete() {
			if (this.selected.length) {
				...
				PModal.popup({
					...
					this.isAll = false;
					res === 1 ? this.$store.commit('HR_SALARY_PER_LIST_INIT', this.depart_id) : '';
				}).catch(() => {});
			} else {
