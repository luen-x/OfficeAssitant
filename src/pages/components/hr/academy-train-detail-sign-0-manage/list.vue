<template>
	<div class="academy-train-detail-sign-manage">
		<vc-paging
			ref="tableTarget"
			:columns="columns"
...
export default {
	...
	computed: {
		listInfo() {
			return this.$store.state.hrAcademyTrainDetailSign0Manage.listInfo;
		}
	},
	methods: {
		loadData(page, pageSize) {
			...
			return this.request({
				url: 'HR_ACADEMY_TRAIN_DETAIL_SIGN_0_MANAGE_LIST_GET',
				type: 'GET',
				param: {
					...query,
					page,
		...
		handleAllDetel() {
			...
			ModalTip.popup({
				...
				const sign_ids = this.selected.reduce((initVal, currentVal) => {
					initVal.push(currentVal.sign_id);
					return initVal;
				}, []);
				this.$request({
...
<style lang="scss">
.academy-train-detail-sign-manage {
	.__footer {
