...
import { services, workTime } from '@stores/services/hr';
...
export default {
	...
	data() {
		...
		return {
			...
			rule: {
				...
				predict_interview_time: [{ required: true, message: "请选择预计面试时间", trigger: "change" }],
				interview_time: [{ required: true, message: "请选择实际面试时间", trigger: "change" }],
				try_start_time: [
	...
	methods: {
		loadData() {
			...
			this.$router.replace({ path: '/hr/recruit/social/summary/edit', query: { ...this.$route.query, page, pageSize } });
			this.loading = true;
			this.$request({
				...
				this.formData.list.forEach((it, index) => {
					this.formData.list[index].age = +it.age;
					this.formData.list[index].position_id = [it.depart_id, it.position_id];
					this.formData.list[index].try_start_time = [it.try_start_time, it.try_end_time];
					this.formData.list[index].region = [it.province, it.city];
				});
			...
			this.$refs.form.validate(valid => {
				if (valid) {
					...
					list.forEach((it, index) => {
						...
						list[index].age = +it.age;
						let time = it.try_start_time;
						list[index].try_start_time = time[0];
						list[index].try_end_time = time[1];
						list[index].province = it.region[0];
						list[index].city = it.region[1];
					});
					this.$request({
						url: '_HR_RECRUIT_SOCIAL_BATCH_POST',
						type: 'POST',
						param: {
							multi_data: list
						},
...
.v-hr-recruit-batch-edit {
	.ivu-table {
		...
		.ivu-input-number .ivu-input-number-input-wrap input {
