<template>
	<i-modal
		...
		:mask-closable="false"
		title="员工晋升"
		class="v-hr-employee-rank-up-promo-modal"
		width="400"
		@on-ok="handleOk"
		...
		<div class="g-fs-14 _content">
			...
			<i-form
				...
				<i-form-item label="晋升职位：" class="_form-item">
					<div>{{ data.after_position_name }}</div>
				</i-form-item>
...
export default {
	name: "hr-employee-rank-up-promo-modal",
	components: {
		"i-modal": Modal,
		"i-form": Form,
	...
	mounted() {
		...
		// 根据默认角色和职位获取默认团队提成
		this.loadTeamId(this.data.role_id);
	...
	methods: {
		...
		loadTeamId(role_id) {
			this.$request({
				...
				param: {
					role_id,
					position_id: this.data.after_position_id
				},
				loading: false
			}).then((res) => {
					...
					this.$request({ 	// 判断是否离开了积分学分的范围
						...
						if (res.data.is_range === 0) { 	// 离开了
							DelTipModal.popup({
								...
								this.$request({ 	// 晋升
									url: '_HR_EMPLOYEE_UP_PROMO_POST',
									type: "POST",
									param: {
										record_id: this.data.record_id,
										status: 3,
...
export const promoPModal = CreatePortal({}, module.exports.default);
</script>

<style lang="scss" scoped>
.v-hr-employee-rank-up-promo-modal {
	._content {
		margin-top: 14px;
	}
}
