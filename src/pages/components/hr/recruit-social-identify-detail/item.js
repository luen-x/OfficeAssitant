...
import OaQuerySelect from '../recruit-social-identify/common/oa-search-select.vue';
		...
		return {
			...
			columns: [
				...
				},
				{
					...
					minWidth: 196,
					render: (h, params) => {
						const { ditch } = this.formData.formList[params.index];
						return (
							<div>
								{ this.formData.formList[params.index].isEditing != 1
									&& <span>{params.row.introducer_name}</span> }
								{ this.formData.formList[params.index].isEditing == 1
									&& <FormItem prop={ditch == 22 ? `formList.${params.index}.introducer_id` : ''} rules={this.rule.introducer_id}>
										<OaQuerySelect
			...
			this.$refs.form.validate(valid => {
				if (valid) {
					...
					this.$request({
						...
						param: {
							...list[params.index],
							depart_id: list[params.index].depart_id,
							position_id: list[params.index].position_id,
							introducer_id: list[params.index].introducer_id,
							ditch: list[params.index].ditch,
