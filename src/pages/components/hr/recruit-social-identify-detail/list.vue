...
export default {
	...
	data() {
		...
		return {
			...
			rule: {
				...
				introducer_id: [
					{ required: true, type: 'number', message: "转介绍人不能为空", trigger: "blur" }
				],
	...
	methods: {
		loadData() {
			...
			this.$router.replace({ path: '/hr/recruit/social/identify-detail', query: { ...this.$route.query, page, pageSize } });
			this.loading = true;
			this.$request({
