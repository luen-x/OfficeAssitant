		...
		<div class="g-flex g-jc-fe g-m-t-10 g-m-r-20">
			...
			<i-dropdown trigger="hover" @on-click="handleClickMore">
				...
				<i-dropdown-menu slot="list">
					<i-dropdown-item v-if="this.$auth[1256]" name="send">发邮件</i-dropdown-item>
					<i-dropdown-item v-if="this.$auth[1257]" name="list">邮件列表</i-dropdown-item>
				</i-dropdown-menu>
			</i-dropdown>
		</div>
		<!-- <div @click="handleDownload">ccc</div> -->
		<vm-preview
			:data-source="content"
			:mask="false"
			:css="{
	...
	methods: {
		...
		handleClickMore(name) {
			if (name === 'send') {
				...
				this.$router.push(getHashUrl('/hr/recruit/social/summary/elists', { recruitType: 'social' }));
			}
		}
	},
};
