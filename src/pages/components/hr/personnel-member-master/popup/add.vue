<template>
	<i-modal
		...
		:loading="true"
		:mask-closable="false"
		:title="type === 1?'添加徒弟':'师傅推荐'"
		width="680"
		class="v-hr-personnel-member-master-add-mentor"
		@on-ok="handleOk"
		@on-cancel="handleCancel"
	>
		<div class="g-fs-14 g-m-b-10" style="color:#E84C57">选择员工并推荐给三立学府评选“微一案师傅”。</div>
...
export default {
				...
				},
				{
					...
					render: (h, params) => {
						return (
							<div class="g-flex">
								{params.row.status === 3 && (
									<div class="_recommend">推荐中</div>
								)}
								{params.row.status === 4 && (
									<div class="_master">师傅</div>
								)}
								{params.row.status === 0 && (
									<div class="_btn" />
								)}
	...
	methods: {
		...
		loadSaveData() {
			this.$request({
				url: API._HR_PERSONNEL_MEMBER_MASTER_RECOMMEND_MASTER_SAVE_POST,
				type: "POST",
				loading: false,
				param: {
					type: 1,
					staff_id: this.ids.map(item => {
		...
		loadData(page) {
			...
			this.$request({
				url: API._HR_PERSONNEL_MEMBER_MASTER_RECOMMEND_MASTER_LIST_GET,
				type: "GET",
				loading: false,
				param: {
					page: page || 1,
...
<style lang="scss" >
.v-hr-personnel-member-master-add-mentor {
