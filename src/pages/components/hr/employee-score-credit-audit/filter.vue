<template>
	<div class="g-m-t-20">
		<div class="g-flex g-ai-c g-jc-sb">
			...
			<span class="g-red-btn-line" @click="handleGoal">一键通过</span>
		</div>
		<vc-expand 
			ref="expand"
			v-model="show"
...
import { GoalModal } from './popup/goal';
...
export default {
	...
	mounted() {
		...
		this.loadPosition(query.depart_id);
	},
	methods: {
		handleSearch(e) {
			this.$router.replace(getHashUrl(
				'/hr/employee/score/credit/audit', 
				{ 
					...this.formdata,
					position_id: this.position_id,
					depart_id: this.formdata.depart_id && this.formdata.depart_id.join()
				}
			));
		...
		handleChange(depart_id) {
			this.positionArr = [];
			this.formdata.depart_id = depart_id;
