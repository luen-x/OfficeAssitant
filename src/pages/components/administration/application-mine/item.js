...
export default {
	...
	methods: {
		...
		handleApply(params) {
			ApplyModal.popup({ title: "查看申请", eventId: params.row.event_id }).then(res => {
				this.$store.commit("ADMINISTRATION_APPLICATION_MINE_LIST_INIT");
			});
		},
		handleDel(params) {
		...
		getColumns(type) {
			let columns = [
				...
				},
				{
					...
					render: (h, params) => {
						return h('span', {}, [
							(this.type == 0 || this.type == 1 || this.type == 5) ? h('span', {
							...
							(this.type == 0 || this.type == 1) ? h('span', {
								style: {
									paddingLeft: '3px',
									paddingRight: '3px'
								}
							...
							(this.type == 2 || this.type == 4) ? h('span', {
								class: "g-c-blue-mid g-pointer",
								on: {
									click: () => {
