	...
	>
		...
		>
			<vc-paging
				...
				class="v-finance-customer-elec-list"
				mode="table"
...
export default {
	...
	computed: {
		listInfo() {
			return this.$store.state.financeCustomerElec.listInfo;
		}
	},
	methods: {
		loadData(page, pageSize) {
			...
			return this.request({
				url: 'FINANCE_CUSTOMER_ELEC_LIST_GET',
				type: 'GET',
				param: {
					...query,
					type: this.type,
				...
				this.resetType('1', this.listInfo.count, '/finance/customer/elec');
			}).catch((error) => {
			});
		},
		handleChange(type) {
			...
			this.$router.replace(getHashUrl('/finance/customer/elec', { ...query }));
		},
