<template>
	...
	>
		<div v-if="list.length==0" class="g-tc">
			...
			<span class="g-c-black7">暂无标记复购标记记录～</span>
		</div>
		<oa-record v-else :records="list">
			<div slot-scope="{data}">
				...
				<div class="g-m-t-5 g-c-black2">
					{{ data.type==1?`复购金额：${data.amount}`:'取消复购' }}
				</div>
			</div>
		</oa-record>
	</i-modal>
...
export default {
	...
	methods: {
		...
		loadData() {
			this.$request({
				url: "_FINANCE_CUSTOMER_REBUY_SIGN_RECORD_GET",
				type: "GET",
				param: {
...
export const RebuySignRecord = CreatePortal({}, module.exports.default);
</script>
<style lang="scss">
</style>