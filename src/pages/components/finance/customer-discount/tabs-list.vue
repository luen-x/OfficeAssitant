	...
	>
		...
		>
			<vc-paging
				...
				class="v-finance-customer-discount-list"
				mode="table"
...
export default {
	...
	computed: {
		listInfo() {
			return this.$store.state.financeCustomerDiscount.listInfo;
		}
	},
	methods: {
		loadData(page, pageSize) {
			...
			return this.request({
				url: 'FINANCE_CUSTOMER_DISCOUNT_LIST_GET',
				type: 'GET',
				param: {
					...query,
					type: this.type,
		...
		handleChange(type) {
			...
			this.$router.replace(getHashUrl('/finance/customer/discount', { ...query }));
		},
