...
export default {
	...
	data() {
		...
		let depart_id = !!query.depart_id && query.depart_id.split(",").map(item => {
			return +item;
		});
		return {
			pickerOptionsStart: {
	...
	computed: {
		filterDepart() {
			...
			const saleDepartId = getItem("firstDepartId_" + this.$global.version).res.data.sale;
			return this.departAll.find(item => item.value == saleDepartId).children;
		}
	...
	methods: {
		handleSearch(event) {
			this.$router.replace(
				getHashUrl("/finance/count/staff/sale", {
					...this.$route.query,
					search: this.search,
		...
		handleExport() {
			const url = getHashUrl(API_ROOT.FINANCE_COUNT_STAFF_LIST_GET, {
				// eslint-disable-line
				...this.$route.query,
				month: this.month,
