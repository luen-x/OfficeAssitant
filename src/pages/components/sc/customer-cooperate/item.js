...
import { EffectiveSingleModal } from "./popup/effective-single";
...
export default {
	...
	mounted() {
		this.$vc.on('sc-customer-cooperate-title-change', (res) => {
			this.loadTitleArray(this.type);
		});
	},
	beforeDestroy() {
		this.$vc.off('sc-customer-cooperate-title-change');
	},
	methods: {
		loadTitleArray(type) {
		...
		handleSkip(params, e) {
			...
			DetailDrawer.popup({
				...
				res && this.$store.commit(
					"SC_CUSTOMER_COOPERATE_LIST_RESET",
					{
						type: 4
					}
				);
		...
		handleOperate(params, name, e) {
			e.stopPropagation();
			
			EffectiveSingleModal.popup({
				project_id: Number(params.row.project_id),
				name: "1"
			}).then(res => {
				res && this.$store.commit(
		...
		handleOpenDrawer(row) {
			this.$auth[302] && DetailDrawer.popup({
				type: this.type,
		...
		getColumns(data = { title_show: [], title_hide: [] }, type) {
			let columns = [{
				...
				render: (h, params) => {
					...
					// is_submit 是否提交给设计 0否 1是

					let arr = [];

					+params.row.tags.renew === 1 && arr.push("续费");
					+params.row.is_submit === 1 && arr.push("已提交设计");
					...
					return (
						<div class="g-flex-ac">
							<AutoToolTip 
								...
								theme="dark"
								placement="bottom" 
								labelClass={this.$auth[302] ? "g-c-blue-mid g-pointer" : ""}
							/>
					...
					return (
						...
						扣除成本
							<Explain
								style={{ marginLeft: '2px' }}
								title="扣除的产品成本"
								content="如微商全套的标准价是139800元，1年的成本是15%，则扣除成本是20970"
							/>
						</div>
					);
				},
			...
			},
			{
				...
				minWidth: 110,
				render: (h, params) => {
					if (params.row.check_person) {
						return (
							<div>
								<div>{params.row.check_person.split(",")[0]}</div>
								<div>{params.row.check_person.split(",")[1]}</div>
							</div>
						);
					} else {
						return null;
			...
			},
			{
				...
				minWidth: 160,
				render: (h, params) => {
					if (params.row.check_time) {
						return (
							<div>
								<div>{params.row.check_time.split(",")[0]}</div>
								<div>{params.row.check_time.split(",")[1]}</div>
							</div>
						);
					} else {
						return null;
			...
			if (type == 6) { 
				columnsShow.push({
					...
					minWidth: 100,
					render: (h, params) => {
						if (params.row.valid_type == 1 && this.$auth[315]) {
							return (
								<div class="g-operation g-oneline" onClick={(e) => { this.handleOperate(params, "1", e); }}>
									计为有效单
								</div>
							);
						} else if (params.row.valid_type == 2 && this.$auth[316]) {
							return (
								<div class="g-operation g-oneline" onClick={(e) => { this.handleOperate(params, "0", e); }}>
									取消计入
								</div>
							);
						} else {
							return (
								<div style="color: #000000">已关联</div>
							);
						}
					}
				});
