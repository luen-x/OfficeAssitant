<template>
	<div class="v-sc-customer-project-requirement g-pd-l-20">
		<div v-if="spin">
			...
			<div class="g-m-t-30">
				...
				<div style="margin-right: 50px;">
					<div 
						class="g-m-t-30 g-flex g-jc-sb g-pointer" 
					...
					>
						<div>
							...
							/>
							<span v-if="project_data.project[0].product_name.length < 12" class="g-m-l-10 g-fs-14">
								{{ project_data.project[0].product_name }}
							</span>
							<i-tooltip 
								v-else
								:content="project_data.project[0].product_name"
								:max-width="200"
								placement="bottom"
								transfer
							>
								{{ project_data.project[0].product_name ? project_data.project[0].product_name.slice(0,12) + "..." : "" }}
							</i-tooltip>
							<span class="g-m-l-20">
							...
							{{ project_data.project[0].product_id }} {{ getRenewAmount(project_data.project[0].product_id) }}
						...
						<div v-if="extra">
							<div 
								v-if="setting_data.have_setting" 
								@click="handleSelect($event,setting_data.value)"
							>
								<div v-if="setting_data.type == 1" class="g-m-r-20" style="color: #ea4c5b;">
									<span>开号时间：{{ setting_data.value.start_time }}</span>
									<span class="g-m-lr-5">|</span>
									<span>使用截止时间：{{ setting_data.value.end_time }}</span>
								</div>
								...
								<div 
									...
									@click="handleSelect($event,setting_data.value)"
								>
									<span>拍摄成本：{{ setting_data.value.shot_cost }}</span>
									<span class="g-m-lr-5">|</span>
									<span>配音成本：{{ setting_data.value.dubbing_cost }}</span>
									<span class="g-m-lr-5">|</span>
									<span>素材成本：{{ setting_data.value.material_cost }}</span>
								</div>
							</div>

							<div v-else class="g-m-r-20" style="color: #2496f7;">
								<span class="g-pointer" @click="handleSelect($event)">
									额外设定
								</span>
							</div>
						</div>
			...
			<div v-if="project_data.finance_audit.status" class="g-m-t-30">
				<span 
					class="g-fs-14 g-c-black3 g-inline-block g-lh-16 g-pd-l-10"
					style="border-left: 2px solid #e84854;height: 16px;"
				>
			...
			<div v-if="project_data.cost_record[0].list.length > 1" class="g-m-t-30">
				<span 
					class="g-fs-14 g-c-black3 g-inline-block g-lh-16 g-pd-l-10"
					style="border-left: 2px solid #e84854;height: 16px;"
				>
					...
					>
						...
						>
							<i
								...
								class="iconfont g-fs-10"
							/>
							<span class="g-m-l-20">{{ item.product_name }}</span>
							<span class="g-m-lr-20">
						...
						<vc-expand :ref="`expand_${index}`" v-model="item.show">
							<div class="g-relative" style="background: #FAFAFA;">
								...
								<div v-if="item.status > 1" style="padding-bottom: 40px;margin-left: 8px;">
									<span 
										class="g-block g-c-black2 g-fs-14 g-m-t-20 g-m-b-20"
										style="margin-left: 40px;"
									>
...
import { Spin, Tooltip, Button } from "iview";
...
export default {
	...
	computed: {
		invalid_show() {
			return (this.name === "cooperate" && this.type == 6) || (this.name === "product" && this.type == 3);
		},
		...
		// 是否展示额外设定
		extra() {
			return (this.name === 'cooperate' && (this.type == 3 || this.type == 4 || this.type == 6) && this.$auth[1468]) 
			|| (this.name === 'product' && this.$auth[1473]);
		},
	},

	mounted() {
		this.loadData();
		this.name === 'cooperate' && this.loadSettingData();
		this.loadTagData();
	},

	methods: {
		handleSelect($event, v) {
		...
		loadSettingData() {
			this.$request({
				url: "_SC_CUSTOMER_COOPERATE_OTHER_SETTING_CONTRACT_GET",
				type: "GET",
				param: {
					project_id: this.project_id
				},
...
<style lang="scss">
.v-sc-customer-project-requirement {
	min-height: 300px;
	padding-bottom: 90px;
