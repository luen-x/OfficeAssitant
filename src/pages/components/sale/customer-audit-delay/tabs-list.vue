	...
	>
		...
		>
			<vc-paging
				...
				class="v-sale-customer-audit-delay-list"
				mode="table"
...
export default {
	...
	computed: {
		listInfo() {
			return this.$store.state.saleCustomerAuditDelay.listInfo;
		}
	},
	methods: {
		loadData(page, pageSize) {
			...
			return this.request({
				url: 'SALE_CUSTOMER_AUDIT_DELAY_LIST_GET',
				type: 'GET',
				param: {
					...query,
					type: this.type,
		...
		handleChange(type) {
			...
			this.$router.replace(getHashUrl('/sale/customer/audit/delay', { ...query }));
		},
