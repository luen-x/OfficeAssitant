...
export default {
	...
	computed: {
		listInfo() {
			return this.$store.state.saleFractionCreditSub.listInfo;
		},
		tabs() {
			return this.$store.state.saleFractionCreditSub.tabs;
		},
		initIcSystemId() {
			return this.$store.state.saleFractionCreditSub.ic_system_id;
		}
	},
	watch: {
		ic_system_id(value) {
	...
	methods: {
		...
		loadTabsData() {
			...
			return this.$request({
				...
				if (Array.isArray(res.data)) {
					...
					this.$store.commit('SALE_FRACTION_CREDIT_SUB_TABS_GET_SUCCESS', { data: res.data, ic_system_ids });
				}
		...
		loadData(page, pageSize) {
			...
			return this.request({
				url: "SALE_FRACTION_CREDIT_SUB_LIST_GET",
				type: 'GET',
				loading: false,
				param: {
					...query,
		...
		handleChange(ic_system_id) {
			...
			this.$router.replace(getHashUrl('/sale/fraction/credit/sub', { ...query }));
		},
		handleChangePageSize() {
			this.$store.commit('SALE_FRACTION_CREDIT_SUB_LIST_INIT');
		}
	}
};
