<template>
	<div class="v-sale-fraction-material-bank g-flex g-pd-20" style="min-width:1200px;height: inherit;">
		<oa-dir
			ref="dir"
...
export default {
	name: 'oa-sale-fraction-material-bank',
	components: {
		'oa-dir': Dir,
		'oa-list': List,
		'oa-view': View
	...
	methods: {
		loadDir() {
			this.$request({
				url: "_SALE_LIBRARY_CATEGORY_ALL_GET",
				type: "GET",
				param: {
					need_top: 1,
					is_power: 1
		...
		loadList(param) {
			let url = "_SALE_LIBRARY_MATERIAL_LIST_GET";

			if (param.category_id == 'recent') {
				url = "_SALE_LIBRARY_BROWSE_LIST_GET";
			}
		...
		listFormater(data) {
			data.list = data.list.map(item => ({
				...item,
				id: item.material_id || item.category_id || item.folder_id,
				name: item.material_name || item.folder_name || item.category_name,
				size: item.file_size,
		...
		handleClickFile(item) {
			...
			if (item.autoFocus) {
				if (item.is_folder || item.is_category) {
					this.file = item;
				} else {
		...
		loadFolder(item) {
			this.$request({
				url: "_SALE_LIBRARY_FOLDER_CONTENT_GET",
				type: "GET",
				param: {
		...
		recordView(id) {
			this.$request({
				url: "_SALE_LIBRARY_BROWSE_CREATE_POST",
				type: "GET",
