	...
	>
		...
		>
			<i-form-item label="审核流程：">
				<oa-across-table 
					:data="audit_schedule"
					style="justify-content:flex-start;height:70px;margin-left:-45px;"
				>
					<div 
						slot-scope="item" 
...
export default {
	...
	data() {
		return {
			visible: false,
			audit_schedule: this.auditSchedule || [],
			formData: {// 表单
				reason: this.reason || '',
		...
		handleOk() {
			this.$refs.form.validate((valid) => {
				if (valid) {
					...
					this.$request({
						url: '_SALE_SERVICE_STOP_APPLY_POST',
						type: "POST",
						loading: false,
						param,
						autoTip: true
...
export const ServiceStop = CreatePortal({}, module.exports.default);
