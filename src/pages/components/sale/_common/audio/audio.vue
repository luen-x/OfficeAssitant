	...
	<div class="v-sale-audio">
		<div class="_audio g-flex">
			<i
				...
				@click="handleToggle" 
			/>
			<span v-if="mode == 'default'" class="g-m-lr-5" style="color: #4B4F57;">
			...
			/>
			<div class="g-m-lr-5" style="white-space: nowrap;color: #4B4F57;">
				<span v-if="mode == 'mid'">{{ handleFormat(curTime) }}/</span>
				<span>{{ handleFormat(totalTime) }}</span>
			</div>
		</div>
		<audio v-if="!disabled" ref="audio" :controls="false" preload="metadata">
...
export default {
	...
	beforeDestroy() {
		this.$refs.audio &&	this.$refs.audio.removeEventListener("timeupdate", this.handleTimeUpdate);
	...
	methods: {
		...
		handleToggle() {
			if (this.disabled || isNaN(this.$refs.audio.duration)) return;
			if (this.pause) {
				this.pause = false;
