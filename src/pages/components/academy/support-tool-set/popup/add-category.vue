<template>
	<i-modal
		...
		:mask-closable="false"
		width="400"
		class-name="v-academy-support-tool-set-add-category-modal"
		@on-ok="handleOk"
	...
	>
		...
		>
			<i-form-item label="类型:" prop="kit_type">
				<i-select
					v-model="formData.kit_type"
					transfer
					class="_select-width"
					style="width: 220px;"
					@on-change="handleChangeType"
				>
					<i-option :value="1">销售工具包</i-option>
					<i-option :value="2">服务工具包</i-option>
				</i-select>
			</i-form-item>
	...
	data() {
		return {
			...
			ruleValidate: {
				...
				kit_type: [{ required: true, type: 'number', message: '类型不能为空', trigger: 'change' }],
			}
		};
	},
	beforeCreate() {
	...
	methods: {
		...
		handleCategoryList() {
			return this.$request({
				...
				param: {
					need_top: 1,
					kit_type: this.formData.kit_type
				}
			}).then((res) => {
				let listString = '';
		...
		createAndEdit() {
			...
			if (this.isSend) {
				this.$request({
					url: '_ACADEMY_SUPPORT_TOOL_CATEGORY_SAVE_POST',
					type: 'POST',
					param: {
						...this.formData,
						position_ids
