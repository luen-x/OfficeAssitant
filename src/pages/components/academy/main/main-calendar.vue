<template>
	<div class="v-academy-main-calendar">
		<vc-calendar
			ref="calendar"
			lang="ch"
			...
			>
				<div class="__sech">
					<div class="g-flex-ac g-jc-sb">
						...
						>{{ it.holiday.holiday && it.holiday.lDate }}</span>
						<span
					...
					>
						<div 
							...
							:key="index1" 
							:class="i.has_open && '__selected-item'"
							class="__item"
						>
							...
							/> -->
							<span 
								:class="i.has_open && '__selected-name'" 
								class="__item-name"
							>
								{{ i.activity_name || i.course_name }}
							</span>
						</div>
...
export default {
	...
	data() {
		...
		return {
			MONTH_CN,
			calendar: {},
			date: this.$route.query.date || date,
			classNumber: '',
		};
	},
	computed: {
		year() {
			return Number(moment(this.$route.query.date).format('YYYY'));
		},
		month() {
			return Number(moment(this.$route.query.date).format('MM')) - 1;
		}
	},
	created() {
	},
	mounted() {
		...
		this.setCalendar(this.$route.query.date || curDateStr);
		this.initCalendar();
	},
	methods: {
		initCalendar() {
			this.$nextTick(() => {
				this.$refs.calendar.showYear = +this.year;
				this.$refs.calendar.showMonth = +this.month;
			});
		},
		handlePrev() {
...
<style lang="scss">
.v-academy-main-calendar {
    display: inline-block;
    padding: 0px 0 30px 0;
    color: #333333;
    box-sizing: content-box;
    overflow: hidden;
    .vc-calendar .__calendar-item {
        padding-right: 15px;
    }
    ._month-header {
