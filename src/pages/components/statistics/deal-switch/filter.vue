...
export default {
	...
	data() {
		return {
			query: {
				...
				depart_id: '',
				depart_ids: [],
				start_time: moment().subtract(5, 'months').toDate(),
				end_time: new Date()
			}
		};
	},
	...
	methods: {
		handleSearch: debounce(function (event) {
			...
			this.$router.replace(getHashUrl('/statistics/deal/switch', _query));
		...
		handleStartTimeChange() {
			if (!this.query.start_time) {
				this.query.start_time = moment().subtract(5, 'months').toDate();

			}

			this.query.end_time = moment(this.query.start_time).add(5, 'months').toDate();
			this.handleSearch();
		...
		handleEndTimeChange() {
			...
			this.query.start_time = moment(this.query.end_time).subtract(5, 'months').toDate();
			this.handleSearch();
