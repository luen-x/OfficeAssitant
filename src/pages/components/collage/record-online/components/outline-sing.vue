		...
		<div class="c-outline-sing-info">
			<div v-if="detailData.isEdit || detailData.record_status == 1">
				<i-form 
					...
					<i-form-item label="记录时间：" style="margin-top: -40px" class="g-pd-b-10">
...
import { STATUS, outlineSingInfo } from './constants';
...
export default {
	...
	data() {
		...
		return {
			...
			formValidate: {
				...
				time: JSON.parse(this.detailData.singing_time || '[[null, null]]'),
	...
	computed: {
		detailDataParse() {
			return ({
				...
				remark: this.detailData.remark || '--',
				singing_time: this.handleFormatTime(this.detailData.singing_time)
			});
		}
	},
	watch: {
	...
	methods: {
				...
				if (valid) {
					this.$request({
						...
						param: {
							...
							...this.formValidate,
							singing_time: JSON.stringify(this.handleFormatDateToStr(this.formValidate.time))
						}
					}).then((res) => {
		...
		handleFormatTime(singTime) {
			const parseTime = JSON.parse(singTime);
