...
export default {
	...
	data() {
		...
		return {
			...
			param: {
				...
				self_contract: Number(query.self_contract) || '',
	...
	methods: {
		...
		// 选择表头
		handleSelectTitle() {
			const { title_show: dataShow, title_hide: dataHide } = this.$store.state.collageRecordOutline.title[this.$route.query.type || "1"];

			TableTitle.popup({
				dataShow,
				dataHide,
				saveUrl: "_COLLAGE_BUSINESS_TABLE_SAVE_POST",
				mutation: "COLLAGE_RECORD_OUTLINE_TITLE_CHANGE",
				type: this.$route.query.type || "1",
				scenario: 4
			}).then(() => {
				// 由于后端是根据选了哪些字段就查哪些数据，修改表头后需要重新查一遍
				this.$vc.emit("collage-record-outline-title-change", { type: this.$route.query.type || "1" });
				this.$store.commit("COLLAGE_RECORD_OUTLINE_LIST_INIT", {
					type: this.$route.query.type || "1"
				});
			}).catch(err => {
