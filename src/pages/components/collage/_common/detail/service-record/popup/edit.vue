<template>
	<i-modal 
		...
		:mask-closable="false"
		width="420px" 
		class="v-collage-service-record-edit"
		@on-ok="handleOk"
	...
	>
		...
		>
			...
			<i-form-item 
				...
				label="主持天数："
			>
				<span>{{ formData.hosting_day }}天</span>
			</i-form-item>
			<i-form-item 
				v-if="lessonType === 2 && data.role_type === 3" 
				label="主持人数："
...
export default {
	...
	methods: {
		...
		handleOk() {
			this.$refs.form.validateAndScroll((valid) => {
				if (valid) {
					...
					this.$request({
						url: '_COLLAGE_BUSINESS_RECORD_ADD_POST',
						type: "POST",
						loading: false,
						param,
						autoTip: !(this.lessonType === 2 && this.data.role_type === 4)
					}).then(res => {
						this.$refs.modal.buttonLoading = false;
						this.visible = false;
						if (this.lessonType === 2 && this.data.role_type === 4) {
							this.$emit('sure', {
								msg: res.msg,
								toast_flag: res.data.toast_flag
							});
						} else {
							this.$emit('sure');
						}
		...
		handleLectureTimeChange(value) {
			if (Array.isArray(value)) {
				this.formData.total_duration = this.formData.subject.reduce((init, cur) => {
					...
					const diffTime = (moment(endTime).diff(moment(startTime), 'minute')) || 0;
		...
		handleAddItem(name) {
			if (name === 'subject') {
				if (this.formData[name].length > 9) return;
...
export const ServiceRecordEdit = CreatePortal({}, module.exports.default);

</script>

<style lang="scss">
.v-collage-service-record-edit {
	._icon-btn {
