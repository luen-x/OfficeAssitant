<template>
	<i-modal 
		...
		:mask-closable="false"
		width="680px" 
		class="v-collage-choose-other-role"
	>
		...
		<div slot="footer" class="g-flex g-jc-sb">
			<div class="g-flex g-ai-c g-pd-l-20">
				最多选择{{ count }}个人；已选择：
				
				<i-tooltip 
					v-if="selectsShow.length > 25"
					:content="selectsShow"
					:max-width="300"
					placement="top"
					transfer
				>
					<span class="g-c-blue-mid" style="margin-top: -4px;">{{ selectsShow.substr(0, 25) + '...' }}</span>
				</i-tooltip>
				<span v-else class="g-c-blue-mid" style="margin-top: -4px;">{{ selectsShow }}</span>
			</div>
			<div>
...
<script>
import { Modal, Tabs, TabPane, Button, Input, Tooltip } from 'iview';
...
export default {
	...
	mixins: [
		keepSelect({ rowKey: 'staff_id', typeKey: 'type' }),
		orderModal,
		columns
	],
	props: {
	...
	computed: {
		listInfo() {
			return this.$store.state.collageOtherRole.listInfo;
		},
		tableHeight() {
	...
	watch: {
		selects(val) {
			this.setSelectsShow(val);
		}
	},
	mounted() {
		this.visible = true;
		if (this.listInfo[this.type].data[this.page]) {
			this.$store.commit('COLLAGE_OTHER_ROLE_LIST_INIT');
		}
		setTimeout(() => {
			this.showList = true;
		});
	...
	methods: {
		...
		loadPersonData(page, pageSize) {
			...
			return this.request({
				...
				redirect: 'COLLAGE_OTHER_ROLE_LIST_GET_SUCCESS',
				type: 'GET',
				param: {
					...this.keywords,
					type: this.type,
				...
				if (this.firstLoad) {
					...
					res.data.checked.forEach(staff_id => {
						let staff = res.data.list.find(it => it.staff_id == +staff_id);
						if (staff) {
							this.selects.push(staff);
						}
					});
				}
			}).catch((error) => {
		...
		handleSelect(selection, row) {
			const errMsg = `最多选择${this.count}个${this.roleMap[this.roleType]}`;
			setTimeout(() => {
				if (this.selects.length > this.count) {
					this.selects.pop();
					this.toggleSelect();
		...
		handleOk() {
			this.visible = false;
			this.$emit('sure', {
				staff_id: this.selects.map(it => it.staff_id)
			});
		},
		/**
		 * 点击取消
		...
		setSelectsShow: debounce(function (arr) {
			this.selectsShow = [...arr].map(it => it.staff_name).join();
		}, 50),
		handleRowClick(row, index) {
			this.$refs.tableTarget[0].$refs.tableTarget.toggleSelect(index);
		},
	}
};

export const ChooseOtherRole = CreatePortal({}, module.exports.default);

</script>

<style lang="scss">
.v-collage-choose-other-role {
	.vc-paging {
		margin-bottom: 0px;
		.__footer {
			position: relative;
