<template>
	<div>
		<i-modal
			...
			:loading="true"
			:mask-closable="false"
			class="v-collage-customer-cus-information"
			@on-ok="handleOk"
...
import FromItem, { getRules, getItemType, formatProductDetail, formatCheckValue,
	formatOptions, getOutputValue, getOutputOption } from "../../../../sale/_common/customer/cus-form/cus-form";

export default {
	name: "collage-customer-modify-cus-information-modal",
	...
	methods: {
		async handleOk() {
			...
			const unfinishItem = this.getUnfinishItem();
			if (unfinishItem) {
				this.$Message.warning("请等待" + unfinishItem.component_label + "上传完成");
		...
		handleSave() {
			...
			this.$request({
				url: '_COLLAGE_CUSTOMER_COOPERATE_ORDER_EDIT_POST',
				type: "POST",
				param: {
		...
		loadData() {
			this.$request({
				url: "_COLLAGE_CUSTOMER_COOPERATE_ORDER_INFO_GET",
				type: "GET",
				param: {
					contract_id: this.contract_id,
					project_id: this.project_id
...
<style lang="scss">
.v-collage-customer-cus-information{
	.ivu-modal-body{
		max-height: 388px;
		overflow: auto;
	}
