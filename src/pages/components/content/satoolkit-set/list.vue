<template>
	<vc-paging
		...
		@expand="handleExpand"
	>
		<div v-if="this.$auth[631]" slot="extra" class="g-flex-ac">
			<i-checkbox
				v-model="isAll"
...
export default {
	...
	computed: {
		listInfo() {
			return this.$store.state.contentSatoolkitSet.listInfo;
		}
	},
	methods: {
		loadData(page, pageSize) {
			...
			this.isSelect = false;
			return this.request({
				url: 'CONTENT_SATOOLKIT_SET_LIST_GET',
				type: 'GET',
				param: {
					...query,
					page,
		...
		loadExpandData(opts) {
			...
			this.loading = index;
			return this.$request({
				url: API_ROOT.CONTENT_SATOOLKIT_SET_LIST_GET,
				type: 'GET',
				loading: false,
				param: {
					kit_type: 2,
		...
		handleDelateAll() {
			...
			Confirm.popup({
				...
				// 进行指派
				this.$request({
					url: API_ROOT._CONTENT_SERVICE_CATEGORY_BATCH_DELATE_POST,
					type: 'POST',
					param: {
						category_ids: arr
					}
