<template>
	<i-modal
		v-model="visible"
		:mask-closable="false"
		class-name="v-content-product-newprocess-add-element"
		width="680"
		title="添加元件"
		@on-ok="handleOk"
...
export default {
	...
	methods: {
		...
		fetch() {
			this.$request({
				url: API._CONTENT_PRODUCT_PROCESS_ELEMENT_GET,
				type: "GET",
				loading: false,
				param: {
					type: this.type,
		...
		handleOk() {
			 ...
			 */
			// this.visible = false;
			this.$emit('sure', this.elementList);
		},
		handleCancel() {
			// this.visible = false;
			this.$emit('close');
...
<style  lang="scss">
    .v-content-product-newprocess-add-element {
        ._element {
            width: 150px;
            height: 80px;
