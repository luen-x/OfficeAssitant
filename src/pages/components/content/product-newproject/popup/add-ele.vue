<template>
	<i-modal
		v-model="visible"
		:mask-closable="false"
		class-name="v-content-peoduct-newpeoject-add-element"
		title="添加元件"
		width="680"
		@on-ok="handleOk"
	...
	>
		<div class="g-flex">
			...
			/>
			<vc-debounce-click :tag="Button" type="primary" class="g-m-l-10" @click="handleSearch">搜索</vc-debounce-click>
		</div>
		<div class="g-m-t-20">
...
export default {
	...
	methods: {
		...
		getdata() {
			this.$request({
				url: API._CONTENT_PRODUCT_ELEMENT_GET,
				type: "GET",
				loading: false,
				param: {
					label: this.label,
		...
		handleOk() {
			...
			this.$store.commit('ADD_PRO_COMPONENT', this.elementList);
		},
		handleCancel() {
			// this.visible = false;
			this.$emit('close');
...
<style lang="scss">
    .v-content-peoduct-newpeoject-add-element {
        ._element {
            width: 150px;
            height: 80px;
