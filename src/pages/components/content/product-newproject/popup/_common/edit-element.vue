	...
	>
		<div>
			<i-form ref="from" :model="formItem" :label-width="80" :rules="ruleValidate">
				...
				<i-form-item v-if="formItem.component_type==7||formItem.component_type==9" label="选择类型:" prop="name">
				...
				<i-form-item label="是否必填:" prop="required">
...
import { Modal, Form, FormItem, Input, Button, Tooltip, InputNumber, Checkbox, CheckboxGroup, RadioGroup, Radio } from 'iview';
		...
		handleOk() {
			...
			this.$refs.from.validate((valid) => {
				if (valid) {
					...
					this.$store.commit("EDIT_PRO_COMPONENT", res);
					this.$emit('sure', {});
				} else {
