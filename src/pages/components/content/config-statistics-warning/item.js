...
export default {
					...
					render: (h, { row }) => {
						return (
							<div class="g-operation g-flex">
								{this.$auth[1494] ? < span onClick={() => { this.handleEditWarn(row); }}>编辑</span> : ""}
								<span class="g-m-l-5 g-m-r-5">|</span>
								 {this.$auth[1495] ? <span onClick={() => { this.handleDelRule(row); }}>删除</span> : ""}
	...
	methods: {
		...
		handleResetCur() {
			this.$store.commit('CONTENT_CONFIG_STATISTICS_WARNING_LIST_RESET', { type: this.type });
		},
		handleLinkTo() {
			this.$router.push('/');
		},
