<template>
	...
	>
		...
		>
			...
			<i-form-item label="服务工具包阈值：" prop="service_kit_limit">
				<i-input-number v-model="formData.service_kit_limit" :max="9999" placeholder="请输入阈值" style="width: 220px"/>
			</i-form-item>
			<i-form-item label="销售工具包阈值：" prop="sale_kit_limit">
				<i-input-number v-model="formData.sale_kit_limit" :max="9999" placeholder="请输入阈值" style="width: 220px"/>
			</i-form-item>

			<i-form-item label="素材库阈值：" prop="library_limit">
					...
					<template v-if="formData.notice_method.includes(1)">
						<div v-for="(item,index) in shorters" :key="index">
							...
							<i
								...
								class="icon-add1 iconfont g-c-blue-light g-m-b-10"
								style="display: inline-block;vertical-align: middle;"
	...
	data() {
		return {
			...
			formData: {
				...
				"service_kit_limit": null, // 服务工具阈值
				"sale_kit_limit": null, // 销售工具阈值
