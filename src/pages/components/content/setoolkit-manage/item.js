...
export default {
					...
					render: (h, params) => {
						return h('div', {}, [
							h('span', {
								style: {
									color: '#2397f9',
									display: `${this.$auth[632] ? "inline-block" : "none"}`,
							...
							h('span', {
								style: {
									color: '#2397f9',
									display: `${this.$auth[615] ? "inline-block" : "none"}`,
									padding: '2px 8px',
									cursor: 'pointer'
								},
								on: {
	...
	methods: {
		...
		handleResetCur() {
			this.$store.commit('CONTENT_SETOOLKIT_MANAGE_LIST_RESET', { type: this.type });
		},
		handleLinkTo() {
			this.$router.push('/');
		},
		delateMaterial(id) {
			this.$request({
				url: API_ROOT._CONTENT_MATERIAL_DEL_GET,
				type: 'GET',
				param: {
					kit_material_id: id
				},
