<template>
	...
	>
		<div>
			<i-form ref="from" :model="formItem" :label-width="80" :rules="ruleValidate">
				...
				<i-form-item
					v-if="formItem.component_type!=6&&formItem.component_type!=7"
					label="元件提示:"
					prop="placeholder"
				>
					<i-input
				...
				<i-form-item v-if="formItem.component_type==2||formItem.component_type==3">
					<template slot="label">
				...
				<i-form-item
					v-if="formItem.component_type==4||formItem.component_type==5"
					label="时间类型"
					prop="name"
				>
				...
				<i-form-item v-show="formItem.component_type==6" label="上传示例：" style="width:330px">
					<vc-imgs-picker
						:max="5"
...
import { Modal, Form, Tooltip, FormItem, Input, Button, InputNumber, Checkbox, CheckboxGroup, RadioGroup, Radio } from 'iview';
...
export default {
	...
	watch: {
		reglist(val) {
			if (this.formItem.component_type == 2 || this.formItem.component_type == 3) {
				this.formItem.data = [];
				this.reglist.forEach(ele => {
					this.formItem.data.push(this.rexger[ele]);
				});
			}
		}
	},
	...
	mounted() {
		...
		const com = { ...this.process[this.num] };
		if (com.data != null) {
			com.data = [...com.data];
		}
		this.formItem = com;
		this.formItem.input_limit = Number(this.formItem.input_limit);
		if (com.data != null && this.formItem.data.length > 0) {
		...
		handleOk() {
			...
			this.$refs.from.validate((valid) => {
				if (valid) {
					...
					this.$store.commit("EDIT_CUS_COMPONENT", res);
					this.$emit('sure', {});
				} else {
