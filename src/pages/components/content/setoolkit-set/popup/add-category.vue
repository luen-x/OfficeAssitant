...
export default {
	...
	data() {
		...
		return {
			...
			isSend: true,
			categoryName: '',
			categoryYear: [{ value: 0, label: '所有人可见' }, { value: 1, label: '一年以上' }],
			formData: {
				category_name: '',
				is_display: 1,
				pid: '0',
			...
			ruleValidate: {
				...
				is_display: [
					{ required: true, type: 'number', message: '是否显示不能为空', trigger: 'blur' }
				],
				pid: [
	...
	mounted() {
		...
		if (this.categoryInfo) {
			...
			if (this.categoryInfo.pid !== '') {
				...
				this.formData.is_display = this.categoryInfo.isDisplay;
			} else {
				this.select_year = true;
	...
	methods: {
		...
		handleCheckedClassNode() {
			...
			if (treeArray[treeArray.length - 1]) {
				if (treeArray[0].title === '根类目') {
					...
					if (this.classListArray[0].category_year === 0) {
						this.select_year = true;
					}
				}
			} else {
		...
		createAndEdit() {
			if (this.isSend) {
				this.$request({
					url: API_ROOT._CONTENT_CATEGORY_CREATE_AND_EDIT_POST,
					type: 'POST',
					param: this.formData
				}).then((res) => {
					if (this.categoryInfo) {
